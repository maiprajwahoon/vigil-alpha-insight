  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8">
      <div className="flex items-end justify-between">
        ...
        <button className="inline-flex items-center gap-2 rounded-xl border border-border bg-white/[0.03] px-3 py-2 text-sm hover:bg-white/[0.06]">
          <Plus className="h-4 w-4" strokeWidth={1.6} /> Add company
        </button>
      </div>
      ...
      {empty ? (
        <div className="glass-card grid place-items-center p-24 text-center">
          ...
          <p className="mt-2 max-w-sm text-sm text-muted-foreground">Add your first company from the scanner to begin tracking quality opportunities.</p>
          <Link to="/scanner" className="mt-6 rounded-full bg-white px-5 py-2 text-sm text-background">Open scanner</Link>
        </div>
      ) : (
        <div className="glass-card p-5">
          <SectionHeading title="7 companies" subtitle="Sorted by your preference" />
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-[11px] uppercase tracking-wider text-muted-foreground">
                  ...
                  <th className="font-medium text-right">Alert</th>
                </tr>
              </thead>
              <tbody>
                {items.map((s) => (
                  <tr key={s.ticker} className="border-t border-border transition-colors hover:bg-white/[0.025]">
                    ...
                    <td className="text-right">
                      <button className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-border text-muted-foreground hover:text-foreground hover:bg-white/[0.05]">
